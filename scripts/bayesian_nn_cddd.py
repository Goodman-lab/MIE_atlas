...
with tf.device('/CPU:0'):
    ...
    for target in target_list: 
        ...
        input_cddd = pd.read_csv("/home/cg588/MIE_atlas/data/CDDD_data/input_data/"+target+".csv")
        val_cddd = pd.read_csv("/home/cg588/MIE_atlas/data/CDDD_data/ext_validation/"+target+".csv")
        
        
        X = input_cddd.values
        y = raw_data['P(Act)'].values
        X_val = val_cddd.values
        ...
        # WARNING: This can break the model
        ...
        bnn_test_aspars_r = alea_data_sparsity_pearsonr(bnn, X_train, y_train, X_test, n_neighbors = X_train.shape[0]//20, distance = 'euclidean')
       
        bnn_test_epis, bnn_test_epis_mean, bnn_test_epis_std = bnn.get_epistemic(X_test)
        bnn_test_espars_r = epis_data_sparsity_pearsonr(bnn, X_train, y_train, X_test, n_neighbors = X_train.shape[0]//20, distance = 'euclidean')
        ...
        bnn_val_aspars_r = alea_data_sparsity_pearsonr(bnn, X_train, y_train, X_val, n_neighbors = X_train.shape[0]//20, distance = 'euclidean')
        
        bnn_val_epis, bnn_val_epis_mean, bnn_val_epis_std = bnn.get_epistemic(X_val)
        bnn_val_espars_r = epis_data_sparsity_pearsonr(bnn, X_train, y_train, X_val, n_neighbors = X_train.shape[0]//20, distance = 'euclidean')
        ...
        data_sparsity_plot(bnn, X_train, y_train, X_val, n_neighbors = X_train.shape[0]//20, distance = 'euclidean')
...
'''
with tf.device('/CPU:0'):    
        bnn_val_espars_r = epis_data_sparsity_pearsonr(bnn, X_train, y_train, X_val, n_neighbors = X_train.shape[0]//10, distance = 'euclidean')
        print(bnn_val_espars_r)
'''
#%%
