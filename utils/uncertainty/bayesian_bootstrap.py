...
class BayesianBootstrapRegressor:
    ...
    def predict_central_interval(self, X, alpha=0.05):
        ...
        """
        y_posterior_samples = self.predict_posterior_samples(X)
        return central_credible_interval(y_posterior_samples, alpha=alpha)
    
    def predict_normal_distr(self, X):
        """With enough samples, predictions are normally distributed. Obtain mean and std of this posterior distribution. 
        Parameter X: The observed data, independent variables (matrix like)
        Returns: Mean and standard deviation for sampled posterior for each input (matrix like)
        """        
        y_posterior_samples = self.predict_posterior_samples(X)
        y_mu = np.array([np.mean(r) for r in y_posterior_samples])
        y_sigma = np.array([np.std(r) for r in y_posterior_samples])
